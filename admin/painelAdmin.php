    ...
    <script>
        ...
        async function atualizarDashboard() {
            try {
                ...
                const resposta = await fetch(`/admin/api/relatorios/buscar_metricas.php?mes=${mes}&ano=${ano}`);
                const resultado = await resposta.json();

                if (!resultado.success) return;

                document.getElementById("valor-metricas").innerText =
                    Number(resultado.faturamento_bruto).toLocaleString("pt-BR", {
                        style: "currency",
                        currency: "BRL"
                    });

                document.getElementById("valorTicket").innerText =
                    Number(resultado.ticket_medio).toLocaleString("pt-BR", {
                        style: "currency",
                        currency: "BRL"
                    });

                document.getElementById("valorOcupacao").innerText =
                    `${Number(resultado.taxa_ocupacao).toFixed(1)}%`;
        ...
        async function renderRanking() {
            try {
                ...
                const resposta = await fetch(`/admin/api/relatorios/buscar_ranking.php?mes=${mes}&ano=${ano}`);
                const resultado = await resposta.json();
